...
# TODO: 必须要加注意力
# TODO: 改成 weighed loss
# TODO: 在 shared expert 和 specific expert 之间添加注意力
# TODO: 两个独立的专家网络学习各自特征学习得不够好，需要优化 specific experts，例如改成 Deep FM 或 Wide & Deep，这样可以实现特征交叉
# TODO: 特征优化，一是构造交叉特征（MMoE MoE），调研交叉特征的方法，二是之前有一些特征的数值太小了，容易不稳定，需要考虑其倒数
# TODO: 引入不确定度，噪声学习，稀疏学习，门控单元
# TODO: 把图输出的特征输入到GRU
# TODO: 扩散图卷积，diffusion机制
# TODO: 多任务学习 MMoE PLE ESMM
# TODO: 对比学习与自监督学习、diffusion graph、序列transformer、因果图卷积、VAE
# TODO: 图卷积理论调研
# TODO: 复现一下Dual Attention Networks
# TODO: 把 expert 网络扩展为 AE 或者 VAE
...
class SelfAttentionLayer(nn.Module):
    def __init__(self, dim_X) -> None:
        ...
        self.dim_X = dim_X
        self.projection = nn.Sequential(nn.Linear(dim_X, dim_X), 
                                        # nn.ReLU()
                                        )
    ...
    def forward(self, X):
        ...
        outputs = torch.mul(weights, X)
        outputs = self.layer_norm(outputs)
        return outputs


class AdaptiveDiffusionGraphConvolution(nn.Module):

    # Initialization
    def __init__(self, dim_X, dim_y, dim_hidden, order, node_embedding_size, bias=False):
        super(AdaptiveDiffusionGraphConvolution, self).__init__()
    ...
    def forward(self, X):
        ...
        #     output = self.gc_layer_norm(output)
        #     output_list.append(output)
        #     adaptive_adj = torch.matmul(adaptive_adj, adaptive_adj) 

        adaptive_adj_1st = F.softmax(F.relu(torch.mm(self.node_embeddings, self.node_embeddings.transpose(0, 1))), dim=1)
        adaptive_adj = adaptive_adj_1st
        for _ in range(self.order):
            output = self.spatial_gc(X, adaptive_adj)
            output_list.append(output)
            adaptive_adj = torch.matmul(adaptive_adj, adaptive_adj_1st) 
        
        output_feat = torch.concat(output_list, dim=1)
        output_feat = self.out_fc(output_feat)  # TODO: 修改为加法
        return output_feat
...
class FullyConnectedExpertNetworks(nn.Module):

    # Initialization
    def __init__(self, dim_X, dim_hidden, dim_y):
        super(FullyConnectedExpertNetworks, self).__init__()

        self.self_attn = SelfAttentionLayer(dim_X)
...
class GraphExpertNetworks(nn.Module):

    # Initialization
    def __init__(self, dim_X, dim_hidden, dim_y, node_embedding_size, order):
        super(GraphExpertNetworks, self).__init__()
...
class GatesInteractionControlNetworks(nn.Module):
    ...
    def __init__(self, dim_X, dim_hidden, dim_y, node_embedding_size, order, num_specific_experts, num_shared_experts, is_last=True):
        ...
        self.shared_experts = nn.ModuleList([GraphExpertNetworks(self.dim_X, self.dim_hidden, self.dim_y, self.node_embedding_size, self.order) for _ in range(self.num_shared_experts)])
        self.task1_experts = nn.ModuleList([FullyConnectedExpertNetworks(self.dim_X, self.dim_hidden, self.dim_y) for _ in range(self.num_specific_experts)])
        self.task2_experts = nn.ModuleList([FullyConnectedExpertNetworks(self.dim_X, self.dim_hidden, self.dim_y) for _ in range(self.num_specific_experts)])
        # self.task1_experts = nn.ModuleList([ExpertGraphNetworks(self.dim_X, self.dim_hidden, self.dim_y, self.node_embedding_size, self.order) for _ in range(self.num_specific_experts)])
        # self.task2_experts = nn.ModuleList([ExpertGraphNetworks(self.dim_X, self.dim_hidden, self.dim_y, self.node_embedding_size, self.order) for _ in range(self.num_specific_experts)])
    ...
    def forward(self, X):
        ...
        gate_expert_output_shared = torch.cat((task1_experts_output, task2_experts_output, shared_experts_output), dim=0)
...
class AdaptiveGraphConvolutionalMultiTaskNetworks(nn.Module):
    ...
    def __init__(self, dim_X, dim_y, num_specific_experts, num_shared_experts,
                 ...
                 adj_mode='pearson', graph_reg=0.05, self_con=0.2, scale=0.4, epsilon=0.1, prob='regression'):
    ...
    def forward(self, X):
        ...
        if self.prob == 'regression':
            ...
            # output1 = self.tower1(torch.concat((gate_outputs[0], gate_outputs[1]), dim=-1))
            res_list.append(output1.squeeze())

            output2 = self.tower2(gate_outputs[1])
            # output2 = self.tower2(torch.concat((gate_outputs[0], gate_outputs[2]), dim=-1))
...
class AgcmtnModel(NeuralNetwork):
    ...
    def fit(self, X, y):
        ...
        self.model = AdaptiveGraphConvolutionalMultiTaskNetworks(
            ...
            self.experts_hidden, self.experts_out, self.tower_hidden, self.order, self.node_embedding_size,
            self.args['adj_mode'], self.args['graph_reg'], self.args['self_con'], 
            self.args['scale'], self.args['epsilon'], self.prob)
            
        self.model_create('WMSE' if self.prob == 'regression' else 'CE')
