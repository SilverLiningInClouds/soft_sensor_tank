# RADGCN: Recurrent Adaptive Diffusion Graph Convolutional Networks
...
# Network
class RecurrentAdaptiveDiffusionGraphConvolutionalNetworks(nn.Module):

    # Initialization
    def __init__(self, dim_X, dim_y, dim_hidden=256, order=2, node_embedding_size=128, 
                 rnn=(1024,), gc=(1024,), out_fc=(256, 256),
                 adj_mode='pearson', graph_reg=0.05, self_con=0.2, scale=0.4, epsilon=0.1,
                 prob='regression'):
        super(RecurrentAdaptiveDiffusionGraphConvolutionalNetworks, self).__init__()
        ...
        if prob == 'regression':
            for _ in range(dim_y):
                ...
                for j in range(len(gc)):
                    self.gc[-1].append(AdaptiveDiffusionGraphConvolution(self.net_gc[j], self.net_gc[j + 1], self.dim_hidden, self.order, self.node_embedding_size))
...
class RadgcnModel(NeuralNetwork):
    ...
    def fit(self, X, y):
        ...
        # Model creation
        self.model = RecurrentAdaptiveDiffusionGraphConvolutionalNetworks(
