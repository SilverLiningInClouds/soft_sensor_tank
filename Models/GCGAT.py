# GC-GAT: Graph Convolution & Graph Attention Networks
...
# Network
class GraphConvolutionGraphAttentionNetworks(nn.Module):
    ...
    def __init__(self, dim_X, dim_y, adj_mode='rbf', gc=(1024, ), in_fc=(512, ), gat=(256, ), out_fc=(256, 256), 
        ...
        super(GraphConvolutionGraphAttentionNetworks, self).__init__()
    ...
    def forward(self, X):
        feat_list = []
        res_list = []
        spatial_adj = adjacency_matrix(X.cpu().numpy(), self.adj_mode, self.graph_reg, self.self_con, self.scale, self.epsilon)
        
        # Input GC
        for gc in self.spatial_gc:
            graph_X = gc(X, spatial_adj)
...
class GcgatModel(NeuralNetwork):
    ...
    def fit(self, X, y):
        ...
        # Model creation
        self.model = GraphConvolutionGraphAttentionNetworks(
